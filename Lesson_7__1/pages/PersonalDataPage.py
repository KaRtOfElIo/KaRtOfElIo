...
class PersonalDataPage:
    ...
    def personal_data(self, name: str, last: str, address: str, email: str, phone: str, city: str, country: str, job: str, company: str):
    ...
    @allure.step("Проверка, имеет ли поле ввода на красный цвет, если оно не заполнено")
    ...
    @allure.step("Проверка, имеют ли поля ввода на зеленый цвет, если они заполнены")
    def other_fields_green(self):
        ...
        for field in other_fields:
            ...
            if field_color != 'rgba(209, 231, 221, 1)':
                return False  # Если найдено поле, которое не зеленое
        return True  # Все поля зеленые
